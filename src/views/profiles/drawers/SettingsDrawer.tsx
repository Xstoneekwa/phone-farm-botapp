...
function buildSettingsFromProfileDetails(
  ...
  const followOverrideStatus = readString(settings, ["follow_account_override_status"], "legacy_unclassified");
  const explicitFollowOverride = followOverrideStatus === "explicit";
  const manualFollowDayOverride = explicitFollowOverride ? readOptionalNumber(settings, ["manual_follow_day_cap"]) : null;
  const manualFollowSessionOverride = explicitFollowOverride ? readOptionalNumber(settings, ["manual_follow_session_cap"]) : null;
  ...
  return {
    ...
    follow: {
      ...
      adminOverrideActive: explicitFollowOverride,
      adminOverrideLabel: manualFollowDayOverride !== null || manualFollowSessionOverride !== null
        ...
        : followOverrideStatus === "none_inherits_package" ? "None — inherits package" : "Legacy value — provenance unclassified",
  ...
  return (
    ...
    >
      ...
      {activeTab === "Follow" ? <div className="settings-grid">
        ...
        <Section title="Configured account limits" badge={follow.adminOverrideActive ? "account override" : "inherits package"} tone={follow.adminOverrideActive ? "warning" : "success"}><Field label="Account override" value={follow.adminOverrideLabel} /><Field label="Current configured limits" value={`${follow.manualFollowDayCap}/day · ${follow.manualFollowSessionCap}/session`} /><NumberField label="Follow cap/day" value={follow.manualFollowDayCap} max={follow.packageFollowDayCap} onChange={(value) => setFollowDraft({ ...follow, manualFollowDayCap: value, adminOverrideActive: true, adminOverrideLabel: `${value}/day · ${follow.manualFollowSessionCap}/session`, capSource: "manual" })} /><p className="muted">Persistent explicit account override. It may be lowered but cannot exceed the package maximum.</p><NumberField label="Follow cap/session" value={follow.manualFollowSessionCap} max={follow.packageFollowSessionCap} onChange={(value) => setFollowDraft({ ...follow, manualFollowSessionCap: value, adminOverrideActive: true, adminOverrideLabel: `${follow.manualFollowDayCap}/day · ${value}/session`, capSource: "manual" })} /><p className="muted">Warmup is temporary and never replaces this field.</p><Field label="Legacy compatibility (read-only, non-authoritative)" value={follow.legacyFollowCapLabel} /></Section>
