...
function UnfollowPill({ current, cap }: { current: number; cap?: number | null }) {
  return <span className={actionPillClass("unfollow")} title={`${current} verified today · daily cap ${typeof cap === "number" ? cap : "unavailable"}`}>{current} <small>cap {typeof cap === "number" ? cap : "—"}</small></span>;
}
              ...
              <tbody>
                {data.days.length ? data.days.map((day) => (
                  <tr key={day.date}>
                    ...
                    <td><UnfollowPill current={day.unfollow_count} cap={day.unfollow_cap} /></td>
